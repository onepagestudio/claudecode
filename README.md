...
3. 進入 **Messaging API** 分頁：
   ...
   - 開啟 **Use webhook**
4. 用手機 LINE App 傳一則文字訊息測試，應該會收到一則以「此為自動回覆...」開頭的回覆
...
- **回覆前的品牌提示語**（目前是「此為自動回覆...」）：編輯 `relay-server.js` 裡的 `DEFAULT_BOT_LABEL` 常數。
...
**更方便的做法：一鍵網址。** 登入 `/admin` 後，頁面最下面會有兩個已經帶好密碼的網址
（暫停 15 分鐘／立即恢復），把它們加到手機主畫面當捷徑，之後點一下圖示就直接執行，
不用再開瀏覽器登入、按按鈕。

### 自動偵測：客人自己說「客服」

除了上面手動的全域開關，還有一個**自動**、**只針對單一客人**的機制：只要客人的訊息裡出現
「客服」兩個字（例如「我要找客服」），機器人會：

1. 不呼叫 Claude，改回一句固定訊息：「請稍後，我們的團隊會盡快回覆您！」
2. 自動暫停對**這位客人**的 AI 自動回覆 1 小時（其他客人不受影響），讓你有空進來處理

這個機制跟上面的全域暫停開關可以同時運作，關鍵字清單在 `relay-server.js` 的
`HUMAN_REQUEST_KEYWORDS` 常數，暫停時長在 `HUMAN_REQUEST_MUTE_MS`，固定回覆文字在
`HUMAN_HANDOFF_MESSAGE`，要調整直接改這幾個值即可。

> 這是我們技術上唯一能做到的「自動」——LINE 本身不會通知我們「店主已經回覆了」，所以無法偵測
> 你自己的回覆動作；但客人自己打出來的文字內容，我們收得到、也能判斷。

**「立即恢復」擁有最高權限**：不管是全域暫停，還是某位客人自己說「客服」觸發的 1 小時暫停，
只要點「立即恢復 AI 回覆」（或對應的一鍵網址），兩種暫停都會一次全部解除——不用等 1 小時，
也不用區分是哪種暫停造成的。

## 10. 客人問 1:1 訂購流程時，自動傳送流程圖

客人的訊息如果同時符合「提到 1:1（或一比一）」+「提到流程／步驟／怎麼訂／怎麼買／怎麼下單／教學」，
...
與 `ONE_TO_ONE_PROCESS_CAPTION` 常數即可。

## 11. 之後若要完全脫離 EasyStore
